...
sub _get_html_table {
  ...
  foreach my $layer_id (@{$layer_ids}) {
    if ($stage_images->{$layer_id}) {
      ...
      $bg_height = $img_height{$layer_id};
      push (@html, "<img src=\"/static/images/expr_viewer/".$stage_images->{$layer_id}."\" style=\"float:left; position:absolute; width: $bg_width px; height: $bg_height px; \">\n");
    }
  }
  ...
  foreach my $ordinal (sort keys %layer_ordinal) {
    ...
    if ($tissue_images->{$layer_id}) {
      $bg_width = $img_width{$layer_id};
      $bg_height = $img_height{$layer_id};
      push (@html, "<img class=\"project_img\" src=\"/static/images/expr_viewer/".$tissue_images->{$layer_id}."\" style=\"width: $bg_width px; height: $bg_height px; float:left; position:absolute; -webkit-filter : brightness(45%) sepia(1) hue-rotate(".$color_deg."deg) saturate(300%); filter : brightness(35%) sepia(1) hue-rotate(".$color_deg."deg) saturate(600%);\">\n");
      $color_deg += 45;
      # $color_deg += 60;
    }
  }
